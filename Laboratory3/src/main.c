...
#define CONFIG_EXAMPLE_FIRMWARE_UPGRADE_URL "https://192.168.245.215:5000/firmware.bin" 
#define CONFIG_FIRMWARE_VERSION_URL "https://192.168.89.22:5000/version"
...
esp_err_t check_firmware_version(const char *version_url)
{
    ...
    ESP_LOGI(TAG, "Checking firmware version from server...");

    // Perform the HTTP GET request to fetch version info
    esp_err_t err = esp_http_client_perform(client);
    
    if (err == ESP_OK) {
        char *response = (char *)esp_http_client_read_response(client);
        ESP_LOGI(TAG, "Firmware version on server: %s", response);

        const char *current_version = "1.0.0"; 
        if (strcmp(response, current_version) > 0) {
            ESP_LOGI(TAG, "New version available, proceeding with OTA.");
        ...
        ESP_LOGE(TAG, "Failed to check firmware version, error: %s", esp_err_to_name(err));
        esp_http_client_cleanup(client);
        return err;
    }
}
...
static void ota_task(void *pvParameters)
{
    ...
    esp_err_t version_check_result = check_firmware_version(CONFIG_FIRMWARE_VERSION_URL);
    if (version_check_result == ESP_OK) {
        esp_http_client_config_t config = {
        ...
        ESP_LOGI(TAG, "No new version available, skipping OTA.");
    }
